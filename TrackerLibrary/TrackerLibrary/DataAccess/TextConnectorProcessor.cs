...
namespace TrackerLibrary.DataAccess.TextHelpers
{
    public static class TextConnectorProcessor
    {
        ...
        {
            List<string> lines = new List<string>();
            foreach (TournamentModel item in models)
            {
                lines.Add($"{item.Id},{item.TournamentName},{item.EntryFee},{ConvertTeamListToString(item.EnteredTeams)},{ConvertPrizeListToString(item.Prizes)},{ConvertRoundListToString(item.Rounds)}");
        ...
        {
            List<string> teams =
                GlobalConfig.TeamFile.FullFilePath().LoadFile();
        ...
        public static void SaveMatchupToFile(this MatchupModel matchup,string matchupFile,string matchupEntryFile)
        {
            ...
            {
                entryModel.SaveEntryToFile(matchupEntryFile);
            }
        ...
        private static string ConvertRoundListToString(List<List<MatchupModel>> rounds)
        {
            ...
            foreach (List<MatchupModel> item in rounds)
            {
                output += $"{ConvertMatchupListToString(item)}|";
            }
        ...
        private static string ConvertPrizeListToString(List<PrizeModel> prizes)
        {
            ...
            foreach (PrizeModel item in prizes)
            {
                output += $"{item.Id}|";
            }
            output = output.Substring(0, output.Length - 1);
        ...
        private static string ConvertMatchupEntryListToString(List<MatchupEntryModel> entries)
        {
            ...
            foreach (MatchupEntryModel item in entries)
            {
                output += $"{item.Id}|";
            }
            output = output.Substring(0, output.Length - 1);
        ...
        private static string ConvertTeamListToString(List<TeamModel>teams)
        {
            ...
            foreach (TeamModel item in teams)
            {
                output += $"{item.Id}|";
            }
            output = output.Substring(0, output.Length - 1);
            ...
            string teamFileName,string peopleFileName,string prizeFileName)
        {
            ...
            List<TeamModel> teams = 
                teamFileName.FullFilePath().LoadFile().ConvertToTeamModels(peopleFileName);
